...
def train_and_evaluate(clf, X_train, y_train, train_df, importance_threshold, target_variable):
    #perform training
    ...
    trimmed_coeffs={key: value for key, value in coeffs.iteritems() if (abs(value)>importance_threshold)}

    print "Trimmed =", trimmed_coeffs
 
    plt.figure()
    plt.bar(range(len(trimmed_coeffs)), trimmed_coeffs.values(), align='center')
